...
const Tarea = ({tarea}) => {
    ...
    const {handleModalEditarTarea, handleModalEliminarTarea, completarTarea, cambiarEstado} = useProyectos()
    ...
    return (
      <div className="border-b p-5 flex justify-between items-center">
          <div className="flex flex-col items-start">
              ...
              {estado && <p className="text-xs font-bold bg-green-600 uppercase rounded-lg text-white p-1">Completado por: {tarea.completado.nombre}</p>}
          ...
          <div className="flex flex-col lg:flex-row gap-2">
            ...
            <button
              className={`${estado ? 'bg-sky-600' : 'bg-gray-600' } px-4 py-3 text-white uppercase font-bold text-sm rounded-lg`} onClick={() => completarTarea(_id)}>{estado ? 'Completa' : 'Incompleta'}
            </button>
